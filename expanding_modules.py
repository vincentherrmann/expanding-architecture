...
class MutatingModule(object):
    ...
    def _split_input_channel(self, channel_number, offset=1):
        ...
        noise_idx = torch.ByteTensor(self.out_channels, self.kernel_size[0], self.kernel_size[1], 1).bernoulli_(0.5).long()
...
class Conv1dExtendable(nn.Conv1d):
    ...
    def _split_output_channel(self, channel_number, offset=1):
        ...
        :param channel_number: The number of the channel that will be split
        :param offset: The offset that the inserted channel will have to the original. For consecutive indices this will be 1.
        '''
        ...
        if self.bias is not None:
            original_bias = self.bias.data
            new_bias = insert_slice(original_bias, original_bias[channel_number:channel_number+1], dim=0, at_index=channel_number + offset)
            self.bias = Parameter(new_bias)
        ...
        # update persistent values
        self.t0_weight[channel_number, :, :] = self.weight.data[channel_number, :, :]
        self.t0_weight = insert_slice(self.t0_weight, self.weight.data[channel_number + offset, :, :], dim=0, at_index=channel_number + offset)
        self.start_ncc[channel_number] = 0
        self.start_ncc = insert_slice(self.start_ncc, torch.zeros(1), dim=0, at_index=channel_number + offset)
        ncc = self.normalized_cross_correlation()
        self.start_ncc[channel_number] = ncc[channel_number]
...
class Flatten2d1d(nn.Module):
    def __init__(self, *args, in_channels, in_h, in_w, out_dimension=1, **kwargs):
...
class Conv2dExtendable(nn.Conv2d):
    ...
    def _split_input_channel(self, channel_number, offset=1):
        ...
        split_positions = ((len(ranks) - ranks.view_as(duplicated_slice)) / len(ranks)) * 2.5 #2.618
        split_positions += torch.zeros(duplicated_slice.size()).uniform_(-0.1, 0.1) # add noise
        #split_positions += torch.zeros(duplicated_slice.size()) + 0.5 # TEST!!! delete this line
        # shuffle positive and negative split positions
        split_tensor = torch.stack([split_positions, (1-split_positions)], dim=3)
        noise_idx = torch.ByteTensor(self.out_channels, self.kernel_size[0], self.kernel_size[1], 1).bernoulli_(0.5).long()
        split1 = torch.gather(split_tensor, dim=3, index=noise_idx)
        split2 = torch.gather(split_tensor, dim=3, index=1-noise_idx)
